...
for parent,dirnames,filenames in os.walk(rootdir):
    for filename in filenames:
        ...
        for i in f.readlines():
            ...
            timelist.append(tmp[5])
        f.close()

        for i in range(0,len(pricelist)-1):
            if(float(pricelist[i]) != 0):
                #后一天小于前一天，说明增股，跌幅达11%以上
                if((float(pricelist[i]) - float(pricelist[i+1])) / (float(pricelist[i])) >= 0.11):
                    try:
                        #获取一个数据库连接，注意如果是UTF-8类型的，需要制定数据库
                        conn=pymysql.connect(host='localhost',user='root',passwd='ede1954b',db='codeigniter',port=3306,charset='utf8')
                        cur=conn.cursor()#获取一个游标
                        cur.execute('update gupiaoinfo set yichang = \'yichang\',yichangdate = \''+ timelist[i] +'\' where gupiaoname =\''+ namelist[0]+'\'')
                        ...
                        print(e.args)
                #后一天大于前一天，说明减股，涨幅达11%以上
                elif((float(pricelist[i]) - float(pricelist[i+1])) / (float(pricelist[i])) <= -0.11):
                    try:
                        #获取一个数据库连接，注意如果是UTF-8类型的，需要制定数据库
                        conn=pymysql.connect(host='localhost',user='root',passwd='ede1954b',db='codeigniter',port=3306,charset='utf8')
                        cur=conn.cursor()#获取一个游标
                        cur.execute('update gupiaoinfo set yichang = \'yichang\',yichangdate = \''+ timelist[i+1] +'\' where gupiaoname =\''+ namelist[0]+'\'')
                        # cur.execute('insert into gupiaoinfo(gupiaoname,gupiaono,latestlowprice,latesthighprice,longlowprice,price,jiazhipaixu,fengxianpaixu) values(\''+list[0]+'\',\''+list[1]+'\',\''+list[3]+'\',\''+list[2]+'\',\''+list[5]+'\',\''+list[4]+'\',\''+str(jiazhipaixu1)+'\',\''+str(fengxianpaixu1)+'\')')
                        conn.commit();
                        cur.close()#关闭游标
                        conn.close()#释放数据库资源
