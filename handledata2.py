...
for parent,dirnames,filenames in os.walk(rootdir):
    for filename in filenames:
        ...
        try:
            ...
            cur.execute('select yichangdate from gupiaoinfo where gupiaono = \'' + gupiaono +'\' and yichang = \'' + 'yichang' +'\'')
            r = cur.fetchall()
            if(len(r) > 0):
                ...
                if(daysdiff(date1sec,date2sec) >= 3):
                    cur.execute('update gupiaoinfo set yichang = \'zhengchang\', yichangdate = \''+ time.strftime("%Y-%m-%d", time.localtime()) +'\' where gupiaono = \''+ gupiaono +'\'')
                conn.commit();
            cur.close()#关闭游标
            conn.close()#释放数据库资源
